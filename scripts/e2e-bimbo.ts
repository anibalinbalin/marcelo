/**
 * BIMBOA E2E using the latest accepted 1Q26 source PDF blob.
 *
...
async function main() {
  ...
  try {
    ...
    const approved = await approveValues(run.id, "e2e-bimbo@local", []);
    ...
    for (const value of values) {
      if (
        value.targetSheet === "PROJ" &&
        BIMBO_PROJ_FORMULA_ROWS.has(value.targetRow)
      ) {
        skippedFormula++;
        continue;
      }
