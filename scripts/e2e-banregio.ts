/**
 * REGIONAL / Banregio E2E using Camila's local source PDF.
 *
...
const COMPANY_ID = 6;
const TICKER = "REGIONAL";
const QUARTER = "4Q25";
const SOURCE_PATH = "public/camila/RA Banregio 4T25.pdf";
...
async function main() {
  ...
  try {
    console.log("uploading REGIONAL source PDF to blob...");
    const sourceBuffer = readFileSync(SOURCE_PATH);
    const blob = await put(
      `reports/${COMPANY_ID}/${QUARTER}/${Date.now()}-banregio-e2e.pdf`,
      sourceBuffer,
    ...
    const approved = await approveValues(run.id, "e2e-banregio@local", []);
